...
# `command` is a shell builtin, so lookups must run through the target shell.
grep -q "run_target \"\$rootfs\" /bin/sh -c 'command -v cage'" "$SCRIPT"
grep -q "run_target \"\$rootfs\" /bin/sh -c 'command -v mpv'" "$SCRIPT"
grep -q "run_target \"\$rootfs\" /bin/sh -c 'command -v python3'" "$SCRIPT"
! grep -q 'run_target "\$rootfs" command -v' "$SCRIPT"
grep -q 'ashipa' "$SERVICE"
grep -q 'XDG_RUNTIME_DIR=/run/user/%U' "$SERVICE"
grep -q 'LIBSEAT_BACKEND=seatd' "$SERVICE"
grep -q 'Restart=on-failure' "$SERVICE"
grep -q 'SupplementaryGroups=video render seat' "$SERVICE"
grep -q 'cage' "$SESSION"
grep -q 'ashipaos-jellyfin-mpv-shim' "$SESSION"
! grep -q 'test-video\\.mp4\\|idle=yes' "$SESSION"
! grep -q 'python3-mpv' "$SCRIPT" "$LOCK" "$CONFIG"
