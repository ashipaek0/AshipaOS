...
[[ -f "$LAYER_DIR/scripts/build-settingsd.sh" ]] && {
  head -1 "$LAYER_DIR/scripts/build-settingsd.sh" | grep -qE '^#!/(bin/bash|usr/bin/env bash)$' && test_result "Shebang" "PASS" || test_result "Shebang" "FAIL"
  grep -qE '^set -E?euo pipefail$' "$LAYER_DIR/scripts/build-settingsd.sh" && test_result "Strict mode" "PASS" || test_result "Strict mode" "FAIL"
