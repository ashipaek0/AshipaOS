...
Dependencies: Layer 1 rootfs, util-linux sfdisk, libguestfs-tools, grub-efi-amd64-bin
EOF
}
...
parse_and_validate_config() {
    ...
    IMAGE_SIZE_MB="$(config_scalar image_size_mb)"
...
populate_image() {
    ...
    [[ "$TARGET" == x86_64 ]] || error "unsupported Layer 2 target: $TARGET"
...
main() {
    ...
    case "${1:-}" in
        ...
        --layout-only) [[ $# -ge 2 ]] || error "--layout-only requires an output file"; mode=layout; layout_path="$2"; shift 2; TARGET="x86_64"; [[ $# -eq 0 ]] || error "--layout-only does not accept a target" ;;
    ...
    ROOTFS_IMAGE="$1"
    TARGET="x86_64"
    [[ $# -eq 1 ]] || error "x86_64 image builder accepts only the rootfs archive"
    parse_and_validate_config
