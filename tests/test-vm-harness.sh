...
print(['ASHIPAOS_INSTALL_OK','ASHIPAOS_BOOT_OK',guard][index-1],flush=True)
PY
chmod +x "$tmp/bin/"*
...
assert manifest['result']=='PASS' and len(stages)==3
assert all('-nic none' in s for s in stages)
assert 'media=cdrom' not in stages[1]
assert 'media=cdrom' in stages[0] and 'media=cdrom' in stages[2]
assert manifest['stages']==['install','installed','guard']
assert manifest['qemu_version']=='QEMU emulator version mock'
if sys.argv[3]=='uefi':
    assert len(manifest['ovmf_vars'])==3 and manifest['firmware']['sha256']
    assert len(set(map(lambda s:s.split('format=raw,file=')[1].split()[0],stages)))==3
PY
