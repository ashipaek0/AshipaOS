...
find_ovmf() {
    ...
    done < <(find /usr/share -type f \( -iname 'OVMF_CODE*.fd' -o -iname 'OVMF_CODE*.4m.fd' \) -print 2>/dev/null)
    return 1
}
...
OVMF=$(find_ovmf) || die 'OVMF_CODE firmware not found'

sha256sum "$IMAGE" > "$SHA_FILE"
qemu-system-x86_64 --version > "$VERSION_FILE" 2>&1 || true
printf '%q ' qemu-system-x86_64 -machine q35 -accel tcg -cpu max -m 1024 \
    -bios "$OVMF" -drive "format=raw,if=virtio,file=$IMAGE,readonly=on" \
    -nographic -serial "file:$SERIAL_LOG" -monitor none -no-reboot > "$COMMAND_FILE"
printf '\n' >> "$COMMAND_FILE"
...
set +e
: > "$SERIAL_LOG"
qemu-system-x86_64 -machine q35 -accel tcg -cpu max -m 1024 \
    -bios "$OVMF" -drive "format=raw,if=virtio,file=$IMAGE,readonly=on" \
    -nographic -serial "file:$SERIAL_LOG" -monitor none -no-reboot >"$QEMU_LOG" 2>&1 &
QEMU_PID=$!
QEMU_STATUS=0
MARKER_OBSERVED=0
