...
head -c 40 /dev/zero > "$TMP/sm1_s905x3_4g.dtb"
printf 'not an image' | gzip -c > "$TMP/valid-gzip.img.gz"
if bash "$ROOT/build/coreelec/ce2a-inspect-image.sh" "$TMP/bad.img.gz" "$PIN" "$TMP/sm1_s905x3_4g.dtb" "$TMP/out" >/dev/null 2>&1; then echo 'accepted malformed image' >&2; exit 1; fi
if CE2A_DTB_SHA256=$(sha256sum "$TMP/sm1_s905x3_4g.dtb" | cut -d' ' -f1) bash "$ROOT/build/coreelec/ce2a-inspect-image.sh" "$TMP/valid-gzip.img.gz" "$PIN" "$TMP/sm1_s905x3_4g.dtb" "$TMP/out-dtb" >/dev/null 2>&1; then echo 'accepted malformed same-name DTB' >&2; exit 1; fi
